...
class _DenoiseNet(nn.Module):
    def __init__(
        ...
        # observation positional encodings
    ...
    def forward(self, noisy_actions, time, global_cond, need_weights=False):
        # global_cond shape: [B, n_obs_steps, cond_dim]
        
        # 1. encoding global condition
        # Linear 层会自动作用于最后一维: [B, T_obs, D] -> [B, T_obs, H]
        obs_tokens = self.cond_proj(global_cond)
        obs_tokens = obs_tokens.transpose(0, 1) # [T_obs, B, H]
        ...
        time_token = time_enc.unsqueeze(0) # [1, B, H]

        # 4. encoding action chunk
        ac_tokens = self.ac_proj(noisy_actions).transpose(0, 1) # [T_act, B, H]
        ac_tokens = ac_tokens + self.dec_pos[: ac_tokens.size(0)]

        # 5. 全序列拼接 (JiT Core)
        # 序列顺序: [Time (1), Obs (T_obs), Action (T_act)]
        # 总长度 = 1 + n_obs_steps + ac_chunk
        dec_in = torch.cat([time_token, obs_tokens, ac_tokens], dim=0)

        # 6. Transformer Forward
        dec_out = self.decoder(dec_in)

        # 7. Slicing
        # we only need the action tokens' output
        # Action tokens begin at index 1 + n_obs_steps
        start_idx = 1 + obs_tokens.size(0)
        ac_out = dec_out[start_idx:]

        # 8. Final Layer
        eps_out = self.eps_out(ac_out, time_enc) # [T_act, B, ac_dim]

        return eps_out.transpose(0, 1)
...
class DiTModel(nn.Module):
    ...
    def conditional_sample(
        ...
        if global_cond is not None:
            global_cond = global_cond.expand(batch_size, -1,-1).to(
    ...
    def _prepare_global_conditioning(
        ...
        [Batch, n_obs_steps, Feature_Dim]
        """
        batch_size, n_obs_steps = batch[OBS_STATE].shape[:2]
        global_cond_feats = [batch[OBS_STATE]] # [B, T, State_Dim]
        ...
        if self.config.env_state_feature:
            # env_state 通常是 [B, D]，需要扩展到 [B, T, D] 以便拼接
            env_state = batch[OBS_ENV_STATE].unsqueeze(1).expand(-1, n_obs_steps, -1)
            global_cond_feats.append(env_state)

        # Concatenate all global conditioning features.
        # Result: [B, T, Total_Cond_Dim]
