...
class X2VPolicy(PreTrainedPolicy):

    config_class = X2VConfig
    name = "x2v_diffusion"
...
class DiffusionModel(nn.Module):
    def __init__(self, config: X2VConfig):
        super().__init__()
        self.config = config
        ...
        if config.training_noise_sampling == "uniform":
            ...
            affine = torch.distributions.transforms.AffineTransform(loc=s, scale=-s)
            self.noise_distribution = torch.distributions.TransformedDistribution(beta_dist, [affine])
        else:
            raise ValueError(...)
...
class DiffusionConditionalUnet1d(nn.Module):
    ...
    def __init__(self, config: X2VConfig, global_cond_dim: int):
        super().__init__()
    ...
    def forward(self, x: Tensor, timestep: Tensor | float, global_cond=None) -> Tensor:
        """
        Args:
